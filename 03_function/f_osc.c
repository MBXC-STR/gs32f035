...
#if OSC_TX_INT_EN == 1
#ifdef TARGET_GS32
static __interrupt void sciaTxFifoIsr(void);					// 锟斤拷锟斤拷锟叫讹拷
#else
static interrupt void sciaTxFifoIsr(void);					// 锟斤拷锟斤拷锟叫讹拷
#endif
#endif
#ifdef TARGET_GS32
static __interrupt void sciaRxFifoIsr(void);					// 锟斤拷锟斤拷锟叫讹拷
#else
static interrupt void sciaRxFifoIsr(void);					// 锟斤拷锟斤拷锟叫讹拷
#endif
static uint8 SciDataRx(uint8 *buf, uint8 len, uint8 timeout);		// 锟斤拷锟捷斤拷锟秸猴拷锟斤拷
static void SciDataRxDeal(void);							// 锟斤拷锟筋处锟斤拷
static void OscDataQcq(void);								// 锟斤拷锟捷诧拷锟斤拷
static void OscDataTxDeal(void);							// 锟斤拷锟捷凤拷锟斤拷
static uint8 GetOscData(uint8 *databuf);					// 锟斤拷取示锟斤拷锟斤拷锟斤拷锟捷ｏ拷锟斤拷一锟斤拷全通锟斤拷锟斤拷锟斤拷
static void OscSciIoInit(void);								// SCI锟接匡拷IO锟斤拷锟竭筹拷始锟斤拷
#if OSC_DATA_CHECK
...
// 中断向量初始化
	...
	#else
	PieCtrlRegs.PIEIER9.bit.INTx1 = 1;						// 使卸希
	#endif
	#if OSC_TX_INT_EN == 1
		#ifdef TARGET_GS32
		...
		#else
		PieCtrlRegs.PIEIER9.bit.INTx3 = 1;						// 使卸希
		#endif
	#if OSC_TX_INT_EN == 1
		#ifdef TARGET_GS32
...
//	OscConFrameBuf[9] = 
	PieCtrlRegs.PIECTRL.bit.ENPIE = 1;   					// Enable the PIE block锟斤拷使锟斤拷PIE模锟斤拷
